...
class ItemToInterestAggregationRepeat(nn.Module):
    def __init__(self, seq_len, hidden_size, k_interests=5):
        super().__init__()
        self.k_interests = k_interests  # k latent interests
        self.theta = nn.Parameter(torch.randn([hidden_size, k_interests]))
...
class LightRepeatMultiHeadAttention(nn.Module):
    def __init__(
        self,
        n_heads,
        k_interests,
        ...
        super(LightRepeatMultiHeadAttention, self).__init__()
        if hidden_size % n_heads != 0:
            raise ValueError(
                "The hidden size (%d) is not a multiple of the number of attention "
                "heads (%d)" % (hidden_size, n_heads)
        ...
        self.attpooling_key = ItemToInterestAggregationRepeat(
            seq_len, hidden_size, k_interests
        )
        self.attpooling_value = ItemToInterestAggregationRepeat(
            seq_len, hidden_size, k_interests
        )
...
class RepeatMultiHeadAttention(nn.Module):
    def __init__(
        self,
        n_heads,
        k_interests,
        ...
        super(RepeatMultiHeadAttention, self).__init__()
        if hidden_size % n_heads != 0:
            raise ValueError(
                "The hidden size (%d) is not a multiple of the number of attention "
                "heads (%d)" % (hidden_size, n_heads)
    ...
    def forward(self, input_tensor, pos_emb, rep_emb):
        # linear map
        mixed_query_layer = self.query(input_tensor)
        mixed_key_layer = self.key(input_tensor)
        mixed_value_layer = self.value(input_tensor)
...
class LightRepeatTransformerLayer(nn.Module):
    """
    One transformer layer consists of a multi-head self-attention layer and a point-wise feed-forward layer.
    ...
    def __init__(
        ...
        super(LightRepeatTransformerLayer, self).__init__()
        self.multi_head_attention = LightRepeatMultiHeadAttention(
            n_heads,
            k_interests,
            hidden_size,
            seq_len,
...
class RepeatTransformerLayer(nn.Module):
    ...
    def __init__(
        ...
        super(RepeatTransformerLayer, self).__init__()
        self.multi_head_attention = RepeatMultiHeadAttention(
            n_heads,
            k_interests,
            hidden_size,
            seq_len,
    ...
    def forward(self, hidden_states, pos_emb, rep_emb):
        attention_output = self.multi_head_attention(hidden_states, pos_emb, rep_emb)
        feedforward_output = self.feed_forward(attention_output)
        return feedforward_output


class LightRepeatTransformerEncoder(nn.Module):
    r"""One LightTransformerEncoder consists of several LightTransformerLayers.
    ...
    def __init__(
        ...
        super(LightRepeatTransformerEncoder, self).__init__()
        layer = LightRepeatTransformerLayer(
            n_heads,
            k_interests,
            hidden_size,
            seq_len,
...
class RepeatTransformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        super(RepeatTransformerEncoder, self).__init__()
        layer = RepeatTransformerLayer(
            n_heads,
            k_interests,
            hidden_size,
            seq_len,
    ...
    def forward(self, hidden_states, pos_emb, rep_emb, output_all_encoded_layers=True):
        """
        Args:
            hidden_states (torch.Tensor): the input of the TrandformerEncoder
            attention_mask (torch.Tensor): the attention mask for the input hidden_states
        ...
        all_encoder_layers = []
        for layer_module in self.layer:
            hidden_states = layer_module(hidden_states, pos_emb, rep_emb)
            if output_all_encoded_layers:
                all_encoder_layers.append(hidden_states)
        if not output_all_encoded_layers:
            all_encoder_layers.append(hidden_states)
