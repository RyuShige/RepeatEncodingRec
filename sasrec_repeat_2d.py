...
class SASRec_Repeat_2d(torch.nn.Module):
    def __init__(self, user_num, item_num, repeat_num, args):
        super(SASRec_Repeat_2d, self).__init__()
        ...
        self.pos_emb = torch.nn.Embedding(args.maxlen, args.hidden_units*2) # TO IMPROVE
        ...
        self.last_layernorm = torch.nn.LayerNorm(args.hidden_units*2, eps=1e-8)

        for _ in range(args.num_blocks):
            new_attn_layernorm = torch.nn.LayerNorm(args.hidden_units*2, eps=1e-8)
            self.attention_layernorms.append(new_attn_layernorm)

            new_attn_layer =  torch.nn.MultiheadAttention(args.hidden_units*2,
                                                            args.num_heads,
                                                            args.dropout_rate)
            self.attention_layers.append(new_attn_layer)

            new_fwd_layernorm = torch.nn.LayerNorm(args.hidden_units*2, eps=1e-8)
            self.forward_layernorms.append(new_fwd_layernorm)

            if args.ffn:
                new_fwd_layer = FeedForward(args.hidden_units*2, args.hidden_units*4, args.dropout_rate, 'gelu', 1e-12)
            else:
                new_fwd_layer = PointWiseFeedForward(args.hidden_units*2, args.inner_size*2, args.dropout_rate)
    ...
    def log2feats(self, log_seqs, log_repeat, pred=False):
        ...
        seqs = torch.cat((seqs, repeat), -1)
        
        if self.po_enc:
            max_length = log_seqs.shape[1]
            pe = self.positional_encoding(max_length, self.item_emb.embedding_dim*2).to(self.dev)
            seqs += pe
        else:
