...
class SASRec_Repeat_Out(torch.nn.Module):
    def __init__(self, user_num, item_num, repeat_num, args):
        super(SASRec_Repeat_Out, self).__init__()
    ...
    def log2feats(self, log_seqs, log_repeat, pred=False):
        ...
        input_concat = torch.cat((log_feats, repeat), -1)
        log_feats = self.concat_layer(input_concat)
